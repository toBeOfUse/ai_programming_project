...
def read_config() -> dict[str, int | float | str]:
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print(
                f"failed to read command-line argument config file {sys.argv[1]}")
    else:
...
class NeuralNetwork:
    ...
    def create_initial_matrices(self) -> list[np.array]:
        ...
        return [
            create_random_matrix(
                self.neurons_per_hidden_layer, self.input_variables),
            *(
                create_random_matrix(
            ...
            create_random_matrix(self.output_variables,
    ...
    def process_network_layer(self, input_weights: np.array, input_vector: np.array) -> np.array:
        weighted_and_biased = np.dot(
            input_weights, input_vector) + self.neuron_bias
        return np.array([self.threshold_fire(d) for d in weighted_and_biased])

    def process_input(self, input: np.array, debug: bool = True) -> np.array:
        assert len(
            input) == self.input_variables, "incorrect number of inputs given"
        output = input
        if debug:
            print("input:\n", output)
    ...
    def train_network(self, training_data: list[TrainingData], debug_output: bool = True) -> None:
        for i in range(self.max_cycles):
            ...
            for layer_matrix in self.layer_matrices:
                cumulative_input = np.dot(
                    layer_matrix, current_output)+self.neuron_bias
                cumulative_inputs.append(cumulative_input)
                current_output = np.array(
                    [self.threshold_fire(d) for d in cumulative_input])
            ...
            if error < self.learning_error_threshold:
                ...
                derivatives_per_weight = np.dot(
                    cost_gradient_vector, outputs[-2].T)
                ...
                for adjusting_layer_index in range(2, self.hidden_layers+2):
                    ...
                    weight_deltas.insert(
                        0,
                        np.dot(cost_gradient_vector,
                               outputs[-adjusting_layer_index-1].T)
                    )
                    if debug_output:
    ...
    def test_network(self, data: list[TrainingData], debug: bool = True) -> float:
